...
bool sfizz_get_sustain_cancels_release(sfizz_synth_t* synth)
{
    return synth->synth.getResources().getSynthConfig().sustainCancelsRelease;
}

void sfizz_set_num_background_threads(sfizz_synth_t* synth, int num_threads)
{
    (void)synth;
    sfz::config::numBackgroundThreads = num_threads;
}

void sfizz_set_load_in_ram(sfizz_synth_t* synth, bool load_in_ram)
{
    synth->synth.getResources().getFilePool().setRamLoading(load_in_ram);
}

void sfizz_set_stealing_coeffs(sfizz_synth_t* synth, float age_coeff, float power_coeff)
{
    (void)synth;
    sfz::config::stealingAgeCoeff = age_coeff;
    sfz::config::stealingPowerCoeff = power_coeff;
}

void sfizz_set_filter_control_interval(sfizz_synth_t* synth, int interval)
{
    (void)synth;
    sfz::config::filterControlInterval = interval;
}

void sfizz_set_delayed_release_voices(sfizz_synth_t* synth, int num_voices)
{
    (void)synth;
    sfz::config::delayedReleaseVoices = static_cast<unsigned>(num_voices);
}
