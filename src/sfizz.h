 ...
 * redirected from a Member Channel to the Manager Channel while MPE was
 * enabled.
 *
 ...
 * to be honoured only on the Manager Channel. The engine redirects such
 * events to the Manager Channel and increments this counter so hosts can
 * observe spec-violating traffic. Program Change is filtered host-side
 * because `sfizz_send_program_change` does not carry a channel. Counter
 * is not reset by `sfizz_set_mpe_enabled`, polyphony changes, or SFZ
 * reloads.
 *
 * @param synth  The synth.
 */
SFIZZ_EXPORTED_API int sfizz_get_redirected_manager_only_message_count(sfizz_synth_t* synth);
...
SFIZZ_EXPORTED_API bool sfizz_get_sustain_cancels_release(sfizz_synth_t* synth);

/**
 * @brief Set the number of background threads for file loading.
 * @since 1.2.1
 *
 * @param synth        The synth.
 * @param num_threads  The number of background threads.
 */
SFIZZ_EXPORTED_API void sfizz_set_num_background_threads(sfizz_synth_t* synth, int num_threads);

/**
 * @brief Set whether to load all samples in RAM.
 * @since 1.2.1
 *
 * @param synth        The synth.
 * @param load_in_ram  Whether to load in RAM.
 */
SFIZZ_EXPORTED_API void sfizz_set_load_in_ram(sfizz_synth_t* synth, bool load_in_ram);

/**
 * @brief Set the stealing coefficients for the voice manager.
 * @since 1.2.1
 *
 * @param synth       The synth.
 * @param age_coeff   The age coefficient (0.0 to 1.0).
 * @param power_coeff The power coefficient (0.0 to 1.0).
 */
SFIZZ_EXPORTED_API void sfizz_set_stealing_coeffs(sfizz_synth_t* synth, float age_coeff, float power_coeff);

/**
 * @brief Set the filter control interval in frames.
 * @since 1.2.1
 *
 * @param synth    The synth.
 * @param interval The interval in frames.
 */
SFIZZ_EXPORTED_API void sfizz_set_filter_control_interval(sfizz_synth_t* synth, int interval);

/**
 * @brief Set the maximum number of delayed release voices.
 * @since 1.2.1
 *
 * @param synth      The synth.
 * @param num_voices The number of voices.
 */
SFIZZ_EXPORTED_API void sfizz_set_delayed_release_voices(sfizz_synth_t* synth, int num_voices);
