...
{
    // CGO HOTFIX: Force OldestStealer for everything.
    // EnvelopeAndAgeStealer is catastrophically slow when releasing 20+ pedal notes under 64 polyphony
    // because it causes O(N log N) sorts and envelope/power checks per stolen voice.
    for (auto& voice : list_)
